...
import GolfUserRound from '@/components/golf-user-round.vue'
...
function filterCourses() {
  ...
  filteredRoundsList.value = roundsList.value.filter((round: Round) =>
    round.user.first_name.toLowerCase().includes(inputValue.value.toLowerCase()) ||
    round.user.last_name.toLowerCase().includes(inputValue.value.toLowerCase())
  );
    ...
    <div>
      ...
      <div class="grid">
        <div v-for="(rounds, index) in filteredRoundsList" :key="rounds.round_number">
          <golf-user-round :rounds="rounds" @updated-round="updateRound"></golf-user-round>
        </div>
      </div>
    </div>
...
<style scoped>
@import '../style/RoundsMasterPage.css';
</style>
