...
const highestCourseRating = computed(() => {
  return Math.ceil(
    props.courseList.reduce((max, course) => Math.max(max, course.course_rating_18), 0),
  )
})
